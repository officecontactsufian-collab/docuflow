...
export default function SplitPage() {
  ...
  const handleFileSelected = async (files: File[]) => {
    ...
    try {
      ...
      setIsDone(false);
      // Default to selecting the first page
  ...
  const updateRangeString = (indices: Set<number>) => {
    ...
    // Group into ranges for display (e.g., 1, 2, 3 -> 1-3)
    const ranges: string[] = [];
    let start = sorted[0];
    let end = start;
          ...
          {!isDone ? (
            ...
            <div className="max-w-md mx-auto space-y-8 text-center animate-in fade-in slide-in-from-bottom-8">
              <div className="p-12 bg-white border border-white/40 rounded-[3rem] shadow-2xl space-y-8">
                <div className="w-20 h-20 text-green-600 flex items-center justify-center mx-auto bg-green-50 rounded-3xl">
